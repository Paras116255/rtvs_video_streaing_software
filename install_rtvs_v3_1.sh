...
# Remove stale RTVS Nginx configs that claim the same domain.
# nginx -t can still succeed when duplicate server_name entries exist;
# nginx may then route traffic to the wrong upstream.
while IFS= read -r -d '' old_conf; do
    [[ "$old_conf" == "$NGINX_CONF" ]] && continue
    if grep -qE "^[[:space:]]*server_name[[:space:]]+${NEW_DOMAIN//./\\.}([[:space:];]|$)" "$old_conf" 2>/dev/null; then
        rm -f "$old_conf"
        warn "Removed stale Nginx config: $old_conf"
    fi
done < <(find /etc/nginx/conf.d /etc/nginx/sites-enabled -maxdepth 1 -type f -print0 2>/dev/null)
...
# Verify RTVS directly first.
RTVS_HTTP_CODE="$(
    curl -sS -o /dev/null -w '%{http_code}' --max-time 10         "http://127.0.0.1:${RTVS_HTTP_PORT}/" 2>/dev/null || true
)"

if [[ "$RTVS_HTTP_CODE" =~ ^[234][0-9][0-9]$ ]]; then
    ok "RTVS direct HTTP check returned $RTVS_HTTP_CODE."
elif [[ "$RTVS_HTTP_CODE" =~ ^4[0-9][0-9]$ ]]; then
    ok "RTVS is reachable directly (HTTP $RTVS_HTTP_CODE)."
else
    warn "RTVS direct HTTP check returned '${RTVS_HTTP_CODE:-no response}'."
    FAILED=1
fi

# Verify Nginx -> RTVS. A 502 is a real proxy failure.
HTTP_CODE="$(
    curl -sS -o /dev/null -w '%{http_code}' --max-time 10         -H "Host: ${NEW_DOMAIN}"         "http://127.0.0.1/" 2>/dev/null || true
)"
...
if [[ "$TARGET_USER" != "root" ]]; then
    ...
    printf 'For the current shell you can also run: newgrp docker\n'
fi
